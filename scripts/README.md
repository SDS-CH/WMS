...
| 08 | `08_pwa_foundation_schema.sql` | **PWA Foundation** — the scanner app's action/debug trail (`wmsactionlog`, append-only, idempotent batch ingest via the `(deviceid, entryid)` unique guard; read by the ERP's PWA Activity Log screen) | ✅ executed on dev 2026-07-06 | 1 |
| 08 | `08_pwa_foundation_seed.sql` | PWA Foundation seed — the action-log **retention policy** rows in `wmssetting` (`pwalog.retentionDays` 90 / `pwalog.errorRetentionDays` 365; the purge endpoint falls back to the same defaults when missing) | ⚠ authored 2026-07-11 — NOT yet executed on dev | — |
| 08 | `08_pwa_auth_config.sql` | **PWA token expirations** — ⚠ runs against the **HOST AUTH DB** (adds `PwaAccessTokenExpiration` / `PwaRefreshTokenExpiration` to `[dbo].[ERPGlobalConfigs]`, minutes, NULL→global fallback; seeds 1440/10080), once per environment, NOT per agency | ✅ executed on host auth DB 2026-07-06 | — |
